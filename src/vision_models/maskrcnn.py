...
def make_all_stages(depth,stride_in_1x1, norm):
    stages = ResNet.make_default_stages(
                depth=depth,
                stride_in_1x1=stride_in_1x1,
                norm=norm,
            )
    stages[-2].extend(make_compress_stages(1024, 11))
    return stages


def mask_rcnn_model(checkpoint_path):
    model = L(GeneralizedRCNN)(
        backbone=L(ResNet)(
            stem=L(BasicStem)(in_channels=3, out_channels=64, norm="FrozenBN"),
            stages=L(make_all_stages)(
                    depth=50,
                    stride_in_1x1=True,
                    norm="FrozenBN",
                ),
